...
*/
  ...
  uint16_t sinkPort = 9; // port number
  ...
  source.SetAttribute ("MaxBytes", UintegerValue (1000000)); // 1MB size

  ApplicationContainer sourceApps = source.Install (nodes.Get (0)); //install BulkSend App on node 0
  sourceApps.Start (Seconds (0.0));
  sourceApps.Stop (Seconds (10.0));
  // Create a PacketSinkApplication and install it on node 1
  PacketSinkHelper sink ("ns3::TcpSocketFactory", InetSocketAddress (interfaces.GetAddress (1), sinkPort));
  //TCP is used to receive pkts on sinkPort of node 1
  ApplicationContainer sinkApps = sink.Install (nodes.Get (1)); //install sink app on node 1
